...
def cargar_cisco():
    ...
    # Convertir columnas específicas a enteros antes del renombramiento
    enteros = [
        "Reitera1", "Reitera2", "Reitera3",
        "Primeravez", "TG", "ProyectosObservados", "ProyectosRevisados"
    ]
    for col in enteros:
        df_cisco[col] = pd.to_numeric(df_cisco[col], errors='coerce').fillna(0).astype(int)

    # Asegurar que Reiteraciones sea numérica antes de transformarla
    df_cisco["Reiteraciones"] = pd.to_numeric(df_cisco["Reiteraciones"], errors="coerce").fillna(0)

    # Convertir columnas específicas a enteros después de cargar
    enteros = [
        "Reitera1", "Reitera2", "Reitera3",
        "Primeravez", "TG", "ProyectosObservados", "ProyectosRevisados"
    ]
    ...
    # Forzar columnas enteras a int en la fila total
    for col in [
        "N° reitera >=1", "N° reitera >=12", "N° reitera >=4",
        "Primera vez", "Total General", "N° Proy obs", "N° Proyectos Revisados"
    ]:
        if col in total_row:
    ...
    # Eliminar columna Reiteraciones_raw antes de retornar
    df_cisco.drop(columns=["Reiteraciones_raw"], inplace=True)

    # Renombrar columnas al final
    df_cisco.rename(columns={
        "UT": "Unidad Territorial",
        "Reitera1": "N° reitera >=1",
        "Reitera2": "N° reitera >=12",
...
# Funciones comunes de conteo para indicadores 2 al 5
...
# Cargar datos para Indicador 07
@st.cache_data
...
# Indicadores 01 a 05
titulos = [
    "Indicador 01: % de proyectos en ejecución dentro del plazo: N° meses en ejecución/plazo (meses)",
    "Indicador 02: % de rendición de cuentas mensual presentada: importe gasto rendido (presentado)/importe gasto autorizado",
    "Indicador 03: % de ejecución monto aprobado de los proyectos: importe devengado/importe aprobado",
    "Indicador 04: % de ejecución financiera de los proyectos: ejecución financiera programada vs ejecutada (SEJPRO)",
    "Indicador 05: % de ejecución física de los proyectos, según programado"
]

for i, ind in enumerate(["Indicador1", "Indicador2", "Indicador3", "Indicador4", "Indicador5"]):
    with tabs[i]:
        st.markdown(tabla_css, unsafe_allow_html=True)
        st.markdown(f"<h2 style='font-size: 12pt;'>{titulos[i]}</h2>", unsafe_allow_html=True)
        ...
        if i == 0:
            ...
            variable = "Indicador5" if i == 4 else ind
            agrupado = df.groupby("txt_zonal").agg(
                **{
        ...
        zonal_sel = st.selectbox(f"Selecciona una Unidad Territorial para ver proyectos (Indicador {i+1}):", sorted(df["txt_zonal"].unique()), key=f"sel_{i}")
        columnas = ["cod_expediente", ind, "txt_provincia", "txt_distrito", "txt_proyecto"]
        df_zonal = df[df["txt_zonal"] == zonal_sel][columnas].drop_duplicates()

        if i == 0:
            df_zonal.rename(columns={"cod_expediente": "Expediente", ind: f"Indicador {i+1}", "txt_provincia": "Provincia", "txt_distrito": "Distrito", "txt_proyecto": "Proyecto"}, inplace=True)
        else:
            df_zonal[f"Indicador {i+1}"] = (df_zonal[ind] * 100).fillna(0).astype(int).astype(str) + "%"
        ...
        columnas_finales = ["Expediente", f"Indicador {i+1}", "Provincia", "Distrito", "Proyecto"]
        ...
        st.download_button(f"Descargar CSV Indicador {i+1:02}", resultado.to_csv(index=False).encode("utf-8"), f"indicador{i+1:02}_por_zonal.csv", "text/csv")
        st.download_button("Descargar Lista de Proyectos", df_zonal.to_csv(index=True).encode("utf-8"), f"lista_proyectos_ind{i+1}.csv", "text/csv")

# Indicador 06
with tabs[5]:
    st.markdown(tabla_css, unsafe_allow_html=True)
    st.markdown("<h2 style='font-size: 12pt;'>Indicador 06: Observaciones  SISCO (reiteraciones de observaciones)</h2>", unsafe_allow_html=True)
